...
public class SendCustomerEmailPromotion {
	...
	public void sendmail() {
		List<CategoryProductPromotionDTO> produtos = categoryProductService.getPromotionalProducts();
		String htmlEmail = buildHtml(produtos);
		List<String> emails = userClient.getAllEmails();
	...
	private String buildHtml(List<CategoryProductPromotionDTO> produtos) {
	...
	private Object montaMeioHtml(CategoryProductPromotionDTO produto) {
		...
		htmlMeioEdit = htmlMeioEdit.replace("$URLIMAGEM", produto.getUrlImage());
		htmlMeioEdit = htmlMeioEdit.replace("$NOMEPRODUTO", produto.getProductName());
		htmlMeioEdit = htmlMeioEdit.replace("$PRECO", produto.getPromotionalPrice().toString());
		return htmlMeioEdit;
	}
